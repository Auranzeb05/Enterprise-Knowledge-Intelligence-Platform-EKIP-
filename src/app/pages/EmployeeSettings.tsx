...
import {
  ...
  Lock, Mail, Eye, Sun, Moon, Globe, Bell, BellOff, ShieldCheck,
  Activity, ChevronDown, Check, AlertCircle, Loader2
} from "lucide-react";
...
export default function EmployeeSettings() {
  ...
  const [email, setEmail] = useState("jamie.reynolds@company.com");
  const [draftEmail, setDraftEmail] = useState("jamie.reynolds@company.com");
  ...
  const [notifs, setNotifs] = useState({ email: true, system: true, digest: false });
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <section>
              ...
              <div style={CARD} className="space-y-0 divide-y-0">
                ...
                <div className="pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                      <button onClick={savePassword} style={BTN_PRIMARY} disabled={isSavingPw}>
                        {isSavingPw ? (
                ...
                <div className="py-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <div className="flex gap-3">
                    <input type="email" value={draftEmail} onChange={(e) => setDraftEmail(e.target.value)}
                      style={{ ...INPUT, flex: 1 }} />
                    <button onClick={saveEmail} style={BTN_PRIMARY} disabled={isSavingEmail}>
                      {isSavingEmail ? (
                        <span className="flex items-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving</span>
                      ) : emailSaved ? (
                        <span className="flex items-center gap-1"><Check className="w-3.5 h-3.5" /> Saved</span>
                ...
                <div className="pt-5">
                  <div className="flex items-center gap-2 mb-4">
                    <Eye className="w-4 h-4" style={{ color: "#2563EB" }} strokeWidth={1.8} />
                  ...
                  <div className="flex gap-2">
                    {(["public", "team", "private"] as const).map((v) => (
                      <button
                        ...
                        style={{
                          background: visibility === v ? "#2563EB" : "rgba(255,255,255,0.04)",
                          border: visibility === v ? "1px solid #2563EB" : "1px solid rgba(255,255,255,0.08)",
                          color: visibility === v ? "#fff" : "#4A6080",
                          cursor: "pointer",
                        }}
                      >
                        {v}
                  ...
                  <p className="text-[11px] mt-2" style={{ color: "#4A6080" }}>
                    {visibility === "public" && "Your profile is visible to everyone in the organisation."}
                    {visibility === "team" && "Your profile is visible to your team members only."}
                    {visibility === "private" && "Your profile is hidden from other users."}
                  </p>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div style={CARD}>
                ...
                <Row label="Email Notifications" hint="Receive important updates via email.">
                  <Toggle on={notifs.email} onChange={(v) => setNotifs((n) => ({ ...n, email: v }))} />
                </Row>

                <Row label="System Notifications" hint="In-app alerts for activity.">
                  <Toggle on={notifs.system} onChange={(v) => setNotifs((n) => ({ ...n, system: v }))} />
                </Row>

                <div className="flex items-start justify-between gap-4 pt-4">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white leading-none">Weekly Digest</p>
                    <p className="text-[11px] mt-1" style={{ color: "#4A6080" }}>Get a weekly summary of knowledge base activity.</p>
                  </div>
            ...
            <div className="flex items-center justify-end gap-4 pt-4 pb-8">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity disabled:opacity-50"
