import React, {
  useEffect,
  useMemo,
  useState,
} from "react";

import { useNavigate } from "react-router";

import {
  LayoutDashboard,
  MessageSquare,
  FileText,
  BarChart2,
  UserCircle,
  RefreshCw,
  Users,
  CheckCircle2,
  Clock3,
  ShieldAlert,
  Building2,
} from "lucide-react";

import { EKIPSidebar } from "../components/Sidebar";
import type { SidebarNavItem } from "../components/Sidebar";
import { useAuth } from "../context/AuthContext";

/* =========================================================
   TYPES
========================================================= */

type UserRole =
  | "employee"
  | "manager"
  | "admin";

type UserStatus =
  | "active"
  | "pending"
  | "suspended";

interface TeamMember {
  id: string;
  fullName: string;
  email: string;
  role: UserRole;
  status: UserStatus;
  createdAt: string;

  department: {
    id: string;
    name: string;
  } | null;
}

interface ManagerDashboardResponse {
  success: boolean;

  manager: {
    id: string;
    fullName: string;
    email: string;
    role: UserRole;
    status: UserStatus;

    department: {
      id: string;
      name: string;

      _count: {
        users: number;
      };
    } | null;
  };

  stats: {
    teamSize: number;
    activeTeamMembers: number;
    pendingTeamMembers: number;
    suspendedTeamMembers: number;
    departmentUsers: number;
  };

  team: TeamMember[];

  message?: string;
}

/* =========================================================
   CONSTANTS
========================================================= */

const API_URL =
  import.meta.env.VITE_API_URL;

const NAV: SidebarNavItem[] = [
  {
    path: "/manager",
    label: "Dashboard",
    icon: LayoutDashboard,
  },
  {
    path: "/ai-chat",
    label: "AI Chat",
    icon: MessageSquare,
  },
  {
    path: "/documents",
    label: "Documents",
    icon: FileText,
  },
  {
    path: "/analytics",
    label: "Analytics",
    icon: BarChart2,
  },
  {
    path: "/manager-profile",
    label: "Profile",
    icon: UserCircle,
  },
];

/* =========================================================
   HELPERS
========================================================= */

function initials(
  name: string
) {
  return name
    .split(" ")
    .filter(Boolean)
    .map(
      (part) => part[0]
    )
    .join("")
    .slice(0, 2)
    .toUpperCase();
}

function formatDate(
  value: string
) {
  const date =
    new Date(value);

  if (
    Number.isNaN(
      date.getTime()
    )
  ) {
    return "—";
  }

  return date.toLocaleDateString(
    undefined,
    {
      day: "numeric",
      month: "short",
      year: "numeric",
    }
  );
}

function formatStatus(
  status: UserStatus
) {
  if (
    status === "active"
  ) {
    return "Active";
  }

  if (
    status === "pending"
  ) {
    return "Pending";
  }

  return "Suspended";
}

function statusColor(
  status: UserStatus
) {
  if (
    status === "active"
  ) {
    return "#22C55E";
  }

  if (
    status === "pending"
  ) {
    return "#F59E0B";
  }

  return "#EF4444";
}

/* =========================================================
   COMPONENT
========================================================= */

export default function ManagerDashboard() {
  const navigate =
    useNavigate();

  const {
    session,
    logout,
  } = useAuth();

  const [
    dashboard,
    setDashboard,
  ] =
    useState<ManagerDashboardResponse | null>(
      null
    );

  const [
    loading,
    setLoading,
  ] = useState(true);

  const [
    error,
    setError,
  ] = useState("");

  /* =======================================================
     LOAD DASHBOARD
  ======================================================= */

  async function loadDashboard() {
    if (
      !session?.access_token
    ) {
      return;
    }

    try {
      setLoading(true);
      setError("");

      const response =
        await fetch(
          `${API_URL}/api/manager/dashboard`,
          {
            headers: {
              Authorization:
                `Bearer ${session.access_token}`,
            },
          }
        );

      const data: ManagerDashboardResponse =
        await response.json();

      if (
        !response.ok
      ) {
        throw new Error(
          data.message ||
            "Failed to load manager dashboard"
        );
      }

      setDashboard(
        data
      );
    } catch (err) {
      console.error(
        "Manager dashboard error:",
        err
      );

      setError(
        err instanceof Error
          ? err.message
          : "Failed to load manager dashboard"
      );
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    if (
      !session?.access_token
    ) {
      return;
    }

    loadDashboard();
  }, [
    session?.access_token,
  ]);

  async function handleLogout() {
    await logout();

    navigate("/", {
      replace: true,
    });
  }

  /* =======================================================
     DERIVED
  ======================================================= */

  const latestTeamMembers =
    useMemo(() => {
      if (
        !dashboard?.team
      ) {
        return [];
      }

      return [
        ...dashboard.team,
      ]
        .sort(
          (a, b) =>
            new Date(
              b.createdAt
            ).getTime() -
            new Date(
              a.createdAt
            ).getTime()
        )
        .slice(
          0,
          6
        );
    }, [
      dashboard,
    ]);

  /* =======================================================
     LOADING
  ======================================================= */

  if (
    loading
  ) {
    return (
      <div
        className="min-h-screen flex items-center justify-center"
        style={{
          background:
            "#0C1525",
        }}
      >
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-2 border-blue-500/30 border-t-blue-500 rounded-full animate-spin" />

          <p
            className="text-sm"
            style={{
              color:
                "#6B7280",
            }}
          >
            Loading manager dashboard...
          </p>
        </div>
      </div>
    );
  }

  return (
    <div
      className="flex h-screen overflow-hidden"
      style={{
        background:
          "#0C1525",

        fontFamily:
          "'Plus Jakarta Sans', Inter, sans-serif",
      }}
    >
      <EKIPSidebar
        items={
          NAV
        }
        subtitle="Manager"
        subtitleColor="rgba(139,92,246,0.5)"
        settingsPath="/manager-settings"
        onLogout={
          handleLogout
        }
      />

      <div className="flex-1 flex flex-col overflow-hidden">

        {/* =================================================
            HEADER
        ================================================= */}

        <div
          className="flex items-center justify-between gap-3 px-4 sm:px-6 py-4"
          style={{
            background:
              "rgba(8,15,28,0.96)",

            borderBottom:
              "1px solid rgba(255,255,255,0.06)",
          }}
        >
          <div>
            <h1 className="text-xl font-bold text-white">
              Manager Dashboard
            </h1>

            <p
              className="text-xs mt-1"
              style={{
                color:
                  "#6B7280",
              }}
            >
              Welcome back
              {dashboard
                ?.manager
                .fullName
                ? `, ${dashboard.manager.fullName}`
                : ""}
              .
            </p>
          </div>

          <button
            onClick={
              loadDashboard
            }
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium"
            style={{
              background:
                "#111D30",

              border:
                "1px solid rgba(255,255,255,0.08)",

              color:
                "#E8EFF8",
            }}
          >
            <RefreshCw className="w-3.5 h-3.5" />

            Refresh
          </button>
        </div>

        {/* =================================================
            CONTENT
        ================================================= */}

        <div className="flex-1 overflow-y-auto p-4 sm:p-6">

          {error && (
            <div
              className="mb-5 px-4 py-3 rounded-xl text-sm"
              style={{
                background:
                  "rgba(239,68,68,0.1)",

                border:
                  "1px solid rgba(239,68,68,0.25)",

                color:
                  "#F87171",
              }}
            >
              {
                error
              }
            </div>
          )}

          {/* =================================================
              MANAGER INFO
          ================================================= */}

          <div
            className="rounded-2xl p-5 mb-6"
            style={{
              background:
                "#111D30",

              border:
                "1px solid rgba(255,255,255,0.06)",
            }}
          >
            <div className="flex items-center gap-4">

              <div
                className="w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold text-white"
                style={{
                  background:
                    "#7C3AED",
                }}
              >
                {dashboard
                  ?.manager
                  .fullName
                  ? initials(
                      dashboard.manager.fullName
                    )
                  : "M"}
              </div>

              <div>
                <h2 className="text-base font-semibold text-white">
                  {dashboard
                    ?.manager
                    .fullName ||
                    "Manager"}
                </h2>

                <p
                  className="text-xs mt-1"
                  style={{
                    color:
                      "#6B7280",
                  }}
                >
                  {dashboard
                    ?.manager
                    .email ||
                    ""}
                </p>
              </div>

              <div className="ml-auto text-right">
                <p
                  className="text-[10px] uppercase tracking-wider font-semibold"
                  style={{
                    color:
                      "#3D5A78",
                  }}
                >
                  Department
                </p>

                <p className="text-sm font-semibold text-white mt-1">
                  {dashboard
                    ?.manager
                    .department
                    ?.name ||
                    "Unassigned"}
                </p>
              </div>
            </div>
          </div>

          {/* =================================================
              KPI CARDS
          ================================================= */}

          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-6">

            <MetricCard
              title="Team Size"
              value={
                dashboard
                  ?.stats
                  .teamSize ??
                0
              }
              icon={
                Users
              }
              color="#60A5FA"
            />

            <MetricCard
              title="Active"
              value={
                dashboard
                  ?.stats
                  .activeTeamMembers ??
                0
              }
              icon={
                CheckCircle2
              }
              color="#4ADE80"
            />

            <MetricCard
              title="Pending"
              value={
                dashboard
                  ?.stats
                  .pendingTeamMembers ??
                0
              }
              icon={
                Clock3
              }
              color="#FBBF24"
            />

            <MetricCard
              title="Suspended"
              value={
                dashboard
                  ?.stats
                  .suspendedTeamMembers ??
                0
              }
              icon={
                ShieldAlert
              }
              color="#F87171"
            />

            <MetricCard
              title="Department Users"
              value={
                dashboard
                  ?.stats
                  .departmentUsers ??
                0
              }
              icon={
                Building2
              }
              color="#A78BFA"
            />
          </div>

          {/* =================================================
              TEAM + DEPARTMENT
          ================================================= */}

          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">

            {/* =============================================
                TEAM MEMBERS
            ============================================= */}

            <div
              className="xl:col-span-2 rounded-2xl overflow-hidden"
              style={{
                background:
                  "#111D30",

                border:
                  "1px solid rgba(255,255,255,0.06)",
              }}
            >
              <div
                className="px-5 py-4"
                style={{
                  borderBottom:
                    "1px solid rgba(255,255,255,0.06)",
                }}
              >
                <h2 className="text-sm font-semibold text-white">
                  Team Members
                </h2>

                <p
                  className="text-xs mt-1"
                  style={{
                    color:
                      "#6B7280",
                  }}
                >
                  Employees directly assigned to you
                </p>
              </div>

              {latestTeamMembers.length ===
              0 ? (
                <div className="py-14 text-center">

                  <Users
                    className="w-8 h-8 mx-auto mb-3"
                    style={{
                      color:
                        "#3D5A78",
                    }}
                  />

                  <p
                    className="text-sm"
                    style={{
                      color:
                        "#6B7280",
                    }}
                  >
                    No employees are assigned to this manager yet.
                  </p>
                </div>
              ) : (
                <div>
                  {latestTeamMembers.map(
                    (
                      member
                    ) => (
                      <div
                        key={
                          member.id
                        }
                        className="flex items-center justify-between px-5 py-4"
                        style={{
                          borderBottom:
                            "1px solid rgba(255,255,255,0.04)",
                        }}
                      >
                        <div className="flex items-center gap-3">

                          <div
                            className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white"
                            style={{
                              background:
                                "#2563EB",
                            }}
                          >
                            {initials(
                              member.fullName
                            )}
                          </div>

                          <div>
                            <p className="text-sm font-semibold text-white">
                              {
                                member.fullName
                              }
                            </p>

                            <p
                              className="text-xs mt-0.5"
                              style={{
                                color:
                                  "#6B7280",
                              }}
                            >
                              {
                                member.email
                              }
                            </p>
                          </div>
                        </div>

                        <div className="text-right">

                          <div className="flex items-center justify-end gap-1.5">

                            <div
                              className="w-1.5 h-1.5 rounded-full"
                              style={{
                                background:
                                  statusColor(
                                    member.status
                                  ),
                              }}
                            />

                            <span
                              className="text-xs font-medium"
                              style={{
                                color:
                                  statusColor(
                                    member.status
                                  ),
                              }}
                            >
                              {formatStatus(
                                member.status
                              )}
                            </span>
                          </div>

                          <p
                            className="text-[10px] mt-1"
                            style={{
                              color:
                                "#3D5A78",
                            }}
                          >
                            Added{" "}
                            {formatDate(
                              member.createdAt
                            )}
                          </p>
                        </div>
                      </div>
                    )
                  )}
                </div>
              )}
            </div>

            {/* =============================================
                DEPARTMENT SUMMARY
            ============================================= */}

            <div
              className="rounded-2xl p-5"
              style={{
                background:
                  "#111D30",

                border:
                  "1px solid rgba(255,255,255,0.06)",
              }}
            >
              <div className="flex items-center gap-2 mb-5">

                <Building2
                  className="w-4 h-4"
                  style={{
                    color:
                      "#A78BFA",
                  }}
                />

                <h2 className="text-sm font-semibold text-white">
                  Department
                </h2>
              </div>

              <p
                className="text-[10px] uppercase tracking-wider font-semibold"
                style={{
                  color:
                    "#3D5A78",
                }}
              >
                Name
              </p>

              <p className="text-lg font-bold text-white mt-1 mb-5">
                {dashboard
                  ?.manager
                  .department
                  ?.name ||
                  "Unassigned"}
              </p>

              <div
                className="rounded-xl p-4 mb-4"
                style={{
                  background:
                    "rgba(139,92,246,0.08)",

                  border:
                    "1px solid rgba(139,92,246,0.15)",
                }}
              >
                <p
                  className="text-[10px] uppercase tracking-wider"
                  style={{
                    color:
                      "#6B7280",
                  }}
                >
                  Department Users
                </p>

                <p
                  className="text-2xl font-bold mt-1"
                  style={{
                    color:
                      "#A78BFA",
                  }}
                >
                  {dashboard
                    ?.stats
                    .departmentUsers ??
                    0}
                </p>
              </div>

              <div
                className="rounded-xl p-4"
                style={{
                  background:
                    "rgba(37,99,235,0.08)",

                  border:
                    "1px solid rgba(37,99,235,0.15)",
                }}
              >
                <p
                  className="text-[10px] uppercase tracking-wider"
                  style={{
                    color:
                      "#6B7280",
                  }}
                >
                  Direct Team
                </p>

                <p
                  className="text-2xl font-bold mt-1"
                  style={{
                    color:
                      "#60A5FA",
                  }}
                >
                  {dashboard
                    ?.stats
                    .teamSize ??
                    0}
                </p>
              </div>
            </div>
          </div>

          {/* =================================================
              REAL DATA NOTICE
          ================================================= */}

          <div
            className="rounded-2xl p-5 mt-5"
            style={{
              background:
                "#111D30",

              border:
                "1px solid rgba(255,255,255,0.06)",
            }}
          >
            <h2 className="text-sm font-semibold text-white">
              Manager Workspace
            </h2>

            <p
              className="text-xs mt-2 leading-5"
              style={{
                color:
                  "#6B7280",
              }}
            >
              Document activity, AI usage, search analytics and knowledge
              metrics will appear here once those EKIP modules are connected.
              This dashboard currently displays only real organization data
              available in the database.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}

/* =========================================================
   METRIC CARD
========================================================= */

function MetricCard({
  title,
  value,
  icon: Icon,
  color,
}: {
  title: string;
  value: number;
  icon: React.ElementType;
  color: string;
}) {
  return (
    <div
      className="rounded-2xl p-4"
      style={{
        background:
          "#111D30",

        border:
          "1px solid rgba(255,255,255,0.06)",
      }}
    >
      <div
        className="w-9 h-9 rounded-lg flex items-center justify-center mb-4"
        style={{
          background:
            `${color}18`,
        }}
      >
        <Icon
          className="w-4 h-4"
          style={{
            color,
          }}
        />
      </div>

      <p
        className="text-[10px] font-semibold uppercase tracking-wider"
        style={{
          color:
            "#3D5A78",
        }}
      >
        {
          title
        }
      </p>

      <p
        className="text-2xl font-bold mt-1"
        style={{
          color:
            "#E8EFF8",
        }}
      >
        {
          value
        }
      </p>
    </div>
  );
}
